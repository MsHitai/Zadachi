...
public class UnixTimePractice {
    ...
    public static void main(String[] args) {
        ...
        Instant dayBefore = examStart.minusSeconds(ONE_DAY_SECONDS);
        if (currentTime.isBefore(dayBefore)) {
        ...
        Instant timeToGoToSleep = timeToWakeUp.minusSeconds((long) sleepGoalHours * ONE_HOUR_SECONDS);
        if (timeToGoToSleep.isAfter(timeToWakeUp)) {
            System.out.println("Такое количество времени поспать уже не получится!");
            return;
        }

        System.out.printf("На учёбу остаётся: %.0f мс.%n", calculateDuration(currentTime, timeToGoToSleep,
                TimeUnits.MILLIS));
        System.out.printf(
