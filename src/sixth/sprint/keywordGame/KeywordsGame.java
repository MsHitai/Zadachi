package sixth.sprint.keywordGame;
...
public class KeywordsGame {
    ...
    public void runGame() {
        final int cases = random.nextInt(KEYWORDS.size());
        int incorrectAnswersCounter = 0;
        while (true) {
            final boolean providedAnswer;
            if (cases < RESERVED.size()) {
    ...
    private boolean writeReservedQuestion() {
        ...
        final int number = random.nextInt(RESERVED.size());
        System.out.println(RESERVED.get(number));
        return true;
    }
